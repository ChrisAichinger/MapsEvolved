...
/*
                        ...
                        GTIFKeySet(gtif, ProjLinearUnitsGeoKey, TYPE_SHORT, 1, Linear_Meter);   
                        GTIFKeySet(gtif, ProjectedCSTypeGeoKey, TYPE_SHORT, 1,   
                                   31257 + (m.getBMNParam().zone-28)/3);    


                        // create 3x3 array of known tiepoints   
                        for (i=0;i<=2;i++) {   
                            for (j=0;j<=2;j++) {   
                                setTiePoint(&m, m.getPixelX()*this_x*i/2,    
                                            m.getPixelY()*this_y*j/2,   
                                            tile_x*m.getPixelX(), tile_y*m.getPixelY(),    
                                            &tp[i*3*6+j*6]);   
                            }   
                        }   
                        // add it to the GeoTIFF   
                        TIFFSetField(image, TIFFTAG_GEOTIEPOINTS, 6*9, tp);   
                        ...
                        // linear unit is meters   
                        GTIFKeySet(gtif, ProjLinearUnitsGeoKey, TYPE_SHORT, 1, Linear_Meter);   
                        // coordinate transformation is Lambert (conformal conic)    
                        // with 2 standard parallels   
                        GTIFKeySet(gtif, ProjCoordTransGeoKey, TYPE_SHORT, 1, CT_LambertConfConic_2SP);   
                        // set projection parameters (origin latitude is always 0 degrees)   
                        GTIFKeySet(gtif, ProjStdParallel1GeoKey, TYPE_DOUBLE, 1, (double)m.getLambertParam().std_par_1);   
                        GTIFKeySet(gtif, ProjStdParallel2GeoKey, TYPE_DOUBLE, 1, (double)m.getLambertParam().std_par_2);   
                        GTIFKeySet(gtif, ProjCenterLongGeoKey, TYPE_DOUBLE, 1, (double)m.getLambertParam().base);   
                        GTIFKeySet(gtif, ProjNatOriginLatGeoKey, TYPE_DOUBLE, 1, (double)0.0);   
